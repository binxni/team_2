...
def frame_points(
    ...
    for ri in return_indices:
        ri_tuple = (int(ri),)
        points_list, _cp_points, _nlz, _intensity, _elongation = _convert_range_image_to_point_cloud_local(
            frame, range_images, camera_projections, range_image_top_pose, ri_index=ri_tuple
        )
        for calib, point_array in zip(calibrations, points_list):
            if allowed_lasers is not None and calib.name not in allowed_lasers:
                continue
            clouds.append(point_array.astype(np.float32, copy=False))
    return clouds
