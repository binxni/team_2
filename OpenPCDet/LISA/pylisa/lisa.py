...
class Lisa:
    ...
    def add_falling_particles(self, pc, Rr, particle_type='rain', roi=None, max_particles=5000):
        """
        Add falling rain/snow particles in the air to simulate dynamic weather effects
        ...
        Returns
        -------
        enhanced_pc : numpy array
            Point cloud with added falling particles [M, 5] with x, y, z, intensity, label
        """
        if Rr <= 0:
            # No precipitation, return original with label 0
            enhanced = np.zeros((pc.shape[0], 5))
            enhanced[:, :4] = pc
            enhanced[:, 4] = 0  # Original points
            return enhanced
            
        # Determine particle density based on rain/snow rate and type
        if particle_type == 'rain':
            # Rain particles - higher density, smaller size
            base_density = Rr * 0.1  # particles per m³
            fall_speed = 5.0  # m/s typical raindrop fall speed
            z_distribution_bias = 1.2  # slightly favor middle-air
        else:  # snow
            # Snow particles - lower density, larger visibility
            base_density = Rr * 0.05  # particles per m³  
            fall_speed = 2.0  # m/s typical snowflake fall speed
            z_distribution_bias = 1.5  # more spread in air

        # 0~10m 반경에서만 노이즈 생성
        r_min, r_max = 0.0, 10.0
        n_particles = min(max_particles, int(base_density * (r_max**3 - r_min**3) * 0.01))

        # ROI 설정: 원본 포인트의 X, Y, Z 범위와 동일하게 제한
        if roi is not None:
        ...
        # 빔 공간 내에서만 직접 샘플링
        n_valid = 0
        max_attempts = n_particles * 10  # 충분히 많은 시도
        sampled = []
        attempts = 0
        while n_valid < n_particles and attempts < max_attempts:
            # 거리: rmin~rmax, 각도: 0~(bdiv/2)
            r = np.random.normal(5.0, 2.0)  # 평균 5m, std=2m
            r = np.clip(r, self.rmin, self.rmax)
            theta = np.random.uniform(0, self.bdiv / 2)
        ...
        falling_particles = np.zeros((n_valid, 5))
        falling_particles[:, 0] = x
        falling_particles[:, 1] = y
        falling_particles[:, 2] = z

        # Intensity 및 라벨
        if particle_type == 'rain':
            base_intensity = np.random.uniform(0.1, 0.3, n_valid)
            falling_particles[:, 4] = 3
        else:
            base_intensity = np.random.uniform(0.2, 0.5, n_valid)
            falling_particles[:, 4] = 4
        distances = np.sqrt(x**2 + y**2 + z**2)
        falling_particles[:, 3] = base_intensity * np.exp(-distances / 30.0)

        # 원본 포인트 + falling particles 합치기
        enhanced = np.zeros((pc.shape[0], 5))
        enhanced[:, :4] = pc
        enhanced[:, 4] = 0
        final_pc = np.vstack([enhanced, falling_particles])
        return final_pc
        
        # Assign intensity based on particle properties and distance
        if particle_type == 'rain':
            # Rain: smaller, less reflective
            base_intensity = np.random.uniform(0.1, 0.3, len(falling_particles))
        else:  # snow
            # Snow: larger, more reflective
            base_intensity = np.random.uniform(0.2, 0.5, len(falling_particles))
            
        # Distance-based intensity decay
        falling_particles[:, 3] = base_intensity * np.exp(-distances / 30.0)
        
        # Label as falling particles (3 for rain, 4 for snow)
        falling_particles[:, 4] = 3 if particle_type == 'rain' else 4
        
        # Combine original points with falling particles
        enhanced = np.zeros((pc.shape[0], 5))
        enhanced[:, :4] = pc
        enhanced[:, 4] = 0  # Original points
    ...
    def augment_with_falling_effects(self, pc, Rr=None, roi=None):
        """
        Enhanced augmentation that includes both original LISA effects and falling particles
        ...
        Rr : float, optional
            Rain rate for rain/snow models
            
        Returns
        -------
        augmented_pc : numpy array
            Augmented point cloud with labels [M, 5]
        """
        if hasattr(self, 'atm_model'):
            if self.atm_model in ['rain', 'snow']:
                if Rr is None:
                    Rr = 10.0  # Default rain rate
                # First apply original LISA augmentation
                lisa_result = self.augment(pc, Rr)
                # ROI는 원본 포인트의 X, Y, Z 범위로 설정
                if roi is None:
                    x_min, x_max = np.min(pc[:, 0]), np.max(pc[:, 0])
                    y_min, y_max = np.min(pc[:, 1]), np.max(pc[:, 1])
                    z_min, z_max = np.min(pc[:, 2]), np.max(pc[:, 2])
                    roi = (x_min, x_max, y_min, y_max, z_min, z_max)
                # Then add falling particles (25~30m shell, ROI 제한)
                enhanced_result = self.add_falling_particles(lisa_result[:, :4], Rr, self.atm_model, roi=roi, max_particles=5000)
                return enhanced_result
            else:
                # For fog models, just apply original augmentation
                fog_result = self.augment(pc)
                enhanced = np.zeros((fog_result.shape[0], 5))
                enhanced[:, :4] = fog_result
                enhanced[:, 4] = 0  # All original/fog-affected points
                return enhanced
        else:
